...
def make_imgs(n_way, k_shot, n_query_per_cls, batchsz,
              support_x, support_y, query_x, query_y, query_pred, device,
	...
	img_support = img_support * torch.FloatTensor(list(std)).view(1, 3, 1, 1).to(device) + \
				  torch.FloatTensor(list(mean)).view(1, 3, 1, 1).to(device)

	img_query = img_query * torch.FloatTensor(list(std)).view(1, 3, 1, 1).to(device) + \
				torch.FloatTensor(list(mean)).view(1, 3, 1, 1).to(device)
